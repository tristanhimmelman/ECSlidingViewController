...
#define kSlidingAnimationDuration 0.3f
...
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    if ([self.delegate respondsToSelector:@selector(slidingViewControllerDidPan)]){
        [self.delegate slidingViewControllerDidPan];
    }
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if ([self underLeftShowing] && currentVelocityX > 0) {
            [self anchorTopViewTo:ECRight withHorizontalVelocity:currentVelocityX animations:nil onComplete:nil];
        } else if ([self underRightShowing] && currentVelocityX < 0) {
            [self anchorTopViewTo:ECLeft withHorizontalVelocity:currentVelocityX animations:nil onComplete:nil];
        } else {
            [self resetTopViewWithHorizontalVelocity:currentVelocityX animations:nil onComplete:nil];
        }
    }
}
...
- (void)anchorTopViewTo:(ECSide)side withHorizontalVelocity:(CGFloat)velocity animations:(void (^)())animations onComplete:(void (^)())complete
{
    ...
    CGFloat deltaX = abs(self.topView.center.x - newCenter);
    NSTimeInterval duration = deltaX / velocity;
    if (duration > kSlidingAnimationDuration){
